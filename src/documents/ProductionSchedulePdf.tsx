...
const ColumnHeaders = () => (
  <View style={styles.tableHeader}>
    ...
    <View
      style={[styles.headerCellBase, styles.colReq, { borderRightWidth: 0 }]}
    >
      <Text style={styles.headerText}>Req</Text>
    </View>
  </View>
);
    ...
    jobs.forEach((job) => {
      ...
      currentPage.push(
        <View style={styles.tableRow} key={job.id} wrap={false}>
          ...
          <View style={[styles.cellBase, styles.colCheck]}>
            <Checkbox checked={Boolean(ps?.panel_completed_actual)} />
          </View>
          <View style={[styles.cellBase, styles.colCheck]}>
          ...
          <View style={[styles.cellBase, styles.colCheck]}>
            <Checkbox
              checked={(() => {
                const doors = !!ps?.paint_doors_completed_actual;
                if (!job.is_canopy_required && !job.is_custom_cab_required) {
                  return doors;
                }
                const canopy =
                  !job.is_canopy_required ||
                  !!ps?.paint_canopy_completed_actual;
                const custom =
                  !job.is_custom_cab_required ||
                  !!ps?.paint_cust_cab_completed_actual;
                return doors && canopy && custom;
              })()}
            />
          </View>
          <View style={[styles.cellBase, styles.colCheck]}>
          ...
          <View
            style={[styles.cellBase, styles.colReq, { borderRightWidth: 0 }]}
          >
            {(job.is_canopy_required ||
              job.is_woodtop_required ||
              job.is_custom_cab_required) && (
              <Text style={styles.reqTag}>
                {[
                  job.is_canopy_required && "CNPY",
                  job.is_woodtop_required && "WT",
                  job.is_custom_cab_required && "CUST",
                ]
                  .filter(Boolean)
                  .join(", ")}
              </Text>
