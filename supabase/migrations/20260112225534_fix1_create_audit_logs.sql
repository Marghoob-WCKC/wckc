...
-- 2. Create the Master Trigger Function
...
DECLARE
  ...
  v_record_id text;
BEGIN
  -- A. Get the User Identifier (Updated for your JWT Template)
  -- Priority: 1. User Role, 2. User ID, 3. Standard Sub, 4. 'system'
  v_user_text := coalesce(
    nullif(current_setting('request.jwt.claim.user_role', true), ''), -- Uses {{user.public_metadata.role}}
    nullif(current_setting('request.jwt.claim.user_id', true), ''),   -- Uses {{user.id}}
    nullif(current_setting('request.jwt.claim.sub', true), ''),       -- Standard Fallback
    'system'
  );
  ...
  ELSE
    -- Default fallback for tables using standard 'id'
